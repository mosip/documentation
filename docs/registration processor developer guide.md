# Registration Processor Developer Guide

## Overview

[Registration Processor (Regproc)](https://docs.mosip.io/1.2.0/modules/registration-processor) is a backend processing engine to enable the ID Lifecycle management. It has several stages and services, registration packet flows through various stages depending on type of flow.
...
For the code setup, clone the repository from [here](https://github.com/mosip/registration.git) and follow the guidelines mentioned in the [Code Contributions](https://docs.mosip.io/1.2.0/community/code-contributions).
...
5. To run the server, two files are required- [kernel-config-server.jar](https://oss.sonatype.org/#nexus-search;gav~~kernel-config-server~1.2.0-SNAPSHOT~~) and [config-server-start.bat](https://github.com/mosip/documentation/blob/1.2.0/docs/_files/registration-processor-config-files/config-server-start.bat).
...
8. Before running any application of Registration Processor replace below properties in bootstrap.properties 
