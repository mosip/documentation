...
**Note:** VCI is supported up to [eSignet v1.4.2](update-link). Going forward, VCI support is provided through **Inji Certify**. Please refer to [Inji Certify](update-link) for the latest implementation.
...
* **Purpose Display Configuration [Link to the Purpose based configuration page](link-update)**
  Clearly indicate the intent of the action—e.g., Login, Verify Identity, or Link Account—to guide user interaction.
* **Multiple Login ID Options [Link to the login configuration page under configure eSignet](link-update)**
