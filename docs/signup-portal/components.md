...
4. **Plugins**: Integration points with external systems are designed to be pluggable, allowing easy integration. The pluggable integration points are as follows:
   * [**Profile Registry Plugin**](integration-guide/profile-registry-plugin.md)- To create and update user identity data
   * [**Identity Verifier Plugin**](integration-guide/identity-verifier-plugin.md) - Plugin to verify the user identity data, basically the backend for the eKYC verification process.
