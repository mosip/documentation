...
1. On January 25th, 2024, the `mosip-compliance-toolkit` tag has been updated from **v1.3.0** to **v1.3.1** to resolve a bug in the `BiometricsQualityCheckValidator` during the conversion of `bioScore` into `bioScoreRange`.
2. Furthermore, on February 12th, 2024, an update was made to the `mosip-compliance-toolkit` tag from **v1.3.1** to **v1.3.2**. This update aims to address the issue of missing table names  `ddl.sql` located within the `db_scripts` folder.
{% endhint %}
