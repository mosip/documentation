...
## Preregistration sync

On every Pre-reg application fetch in registration page, clears all the captured data prior to pre-reg application fetch. Set the field id's which should not be cleared after Pre-reg application fetch. It is comma separated list of field ids as per UI-SPEC.
```
...
Offline jobs, which will be not part of manual sync. Add by comma separator if need to add more jobs
```
...
CRON expression for scheduling the Jobs
```
