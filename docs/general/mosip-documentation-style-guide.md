...
1.  **Language and Grammar**: Use second person in documents rather than first person — _you_ instead of _we_. Avoid using _our_ in documentation. Also, use active voice to clearly indicate who is performing the action.
...
2. **Clarity**: Content needs to be concise. Write short and crisp sentences. Use unordered bullet points to simplify content. If content is in paragraphs, limit them to 5-6 sentences (about 200 words).
...
4.  **Punctuations**: The right use of commas and periods stop is extremely important. Sentences should end with a period, and connecting phrases should have commas.

    Example: For more information on punctuations, refer [here](https://www.grammarly.com/blog/punctuation).
5. **Symbols**: Do not use symbols and shortcuts like ampersand (&) in headings, text, navigation, or tables of contents. Use **and**, unless it is necessary in the code.
...
7.  **Tables**: Use tables to organize data succintly.
