...
_Note_: The mandatory fields/labels have a `*` mark. Field and button labels, error and information messages will be displayed in the user preferred language selected in the login screen. The fields displayed on this screen are configurable based on the [ID schema](../../id-lifecycle-management/id-schema.md) defined by the country.
