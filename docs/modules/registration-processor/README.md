...
Registration Processor (Regproc) is a backend processing engine to enable the ID Lifecycle management. The diagram below shows the Registration Processor along with the other modules that contribute in issuing a Unique Identification Number(UIN) for an individual. Internally, Regproc follows the [SEDA](https://en.wikipedia.org/wiki/Staged\_event-driven\_architecture) architecture where data flows via multiple stages till the UIN is issued.
...
4. Regproc shares biometric data with [ABIS](../../biometrics/abis.md), Manual adjudication System and Verification System. The policy for sharing this data is fetched from [PMS](../partner-management-services/).
