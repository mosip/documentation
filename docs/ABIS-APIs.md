...
| February 05, 2021 | Note on [refernceURL](ABIS-APIs.md#reference-url) and [authentication token](ABIS-APIs.md#authentication-token) was added for Insert Request                                                                                                 |
| March 23, 2021    | New [failure reason](ABIS-APIs.md#failure-reasons) (code - 17) for ABIS has been added.                                                                                                                                                      |
...
The sample encrypted datashare is available [here: encrypted-datashare.txt](\_files/abis/encrypted-datashare.txt)

{% hint style="info" %}
The P12 file for the above encrypted CBEFF file is available [here: cbeff.p12](\_files/abis/cbeff.p12).
...
The data downloaded would be base64 encoded. Hence, after decoding the data will be in the below format. It will be divided in two Parts after splitting using #KEY\_SPLITTER#.
{% endhint %}
...
* The second part is the _**Encrypted Random AES Key**_ which is encrypted with RSA OAEP - SHA256-MFG1. This cosistutes the remaining 256 bytes of Block 1.
...
The data downloaded would be URL safe base64 encoded. Hence, after decoding the data will be in the below format. It will be divided in two Parts after splitting using #KEY\_SPLITTER#.
{% endhint %}
...
* The 3rd part is the _**Encrypted Random AES Key**_, encrypted with the RSA OAEP - SHA256-MFG1. This cosistutes the remaining 256 bytes of Block 1.
