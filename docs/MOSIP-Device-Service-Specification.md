...
{% hint style="info" %}
During the registration of L0 devices please sign using the key that's generated inside the device and send the public key in x509 encoded spec form. <br>
...
* The entire device data is sent in a JWT format. So it will look like this:
```
...
```
{
 ...
    "env": "prod/development/stage"
  ...
  "error": [
    {
      "errorCode": "Error code if registration fails. The remaining keys above are dropped in case of errors.",
...
{% hint style="info" %}
The device once registered for a specific purpose can not be changed after a successful registration. The device can only be used for that specific MOSIP process.
{% endhint %}
...
The device data in the request is sent in a JWT format. So the final request will look like this:
```
"request": {
...
deviceCode - This is the device code issued by the MOSIP server post-registration. This will be in UUID RFC4122 Version 4 format. Once the device is registered the device code needs to be set in the device.
...
1. Organization should have a mechanism to segregate the FTM's and Devices built for MOSIP using a cryptographically valid and repeatable process.
1. All debug options within the FTM or device should be disabled permanently
1. All key creations need for provisioning should happen automatically using FIPS 140-2 Level 3 or higher devices. No individual or a group or organization should have a mechanism to influence this behaviour.
