...
* **Enable MDS** - Below property should updated to enable MDS integration. This property is present in [registration-env.properties](https://github.com/mosip/mosip-config/tree/master/config-templates/registration-env.properties) file - `mosip.mdm.enabled=Y`     
* **Register Biometric Devices for Registration**     
   ...
   * `Whitelist Device` - Biometric Devices can be white-listed using below steps -      
		...
		* Map White-listed Device to a Center ID to which Registration Client is tagged using [Registration Centre Device API](Registration-Center-APIs#post-registrationcenterdevice)   
